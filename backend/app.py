...
async def monthly_series(session: AsyncSession, category: str | None):
    ...
    if category and category != "All":
        stmt = select(month, func.sum(Sale.sales).label("y")) \
                .where(Sale.product_category == category) \
...
async def train_forecast(
    ...
    async def training_task():
        try:
            ...
            m.fit(ts)
            joblib.dump(m, model_path_for(category))
        except Exception as e:
            print(f"Training failed: {e}")
    
    background_tasks.add_task(lambda: asyncio.create_task(training_task()))
